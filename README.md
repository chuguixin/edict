...
<p align="center">
  <sub>12 个核心 AI Agent 组成三省六部：太子分拣/调研、中书省规划、门下省审核封驳、尚书省派发、六部（+吏部）并行执行、钦天监聚合。<br>比 CrewAI 多一层<b>制度性审核</b>，比 AutoGen 多一个<b>实时看板</b>。</sub>
</p>
...
> 🐳 **没有 OpenClaw？** 跑一行 `docker run -p 8080:8080 cft0808/edict` 即可体验完整看板 Demo（预置模拟数据）。
...
### ⚔️ 本项目 (chuguixin/edict) 与原始项目 (cft0808/edict) 的差异

本项目在原版三省六部的强大架构基础上，进行了现代化工程改造，大幅提升了可视化体验和部署便利性：

| 维度 | cft0808/edict (原版) | **chuguixin/edict (本项目)** |
|------|--------------------|----------------------------|
| **可视化视图** | 列表式纯数据展现 | **新增全局「组织架构图 (Org Chart)」**（支持节点点击交互与动态渐变渲染） |
| **首屏体验** | 默认展示任务看板 | **默认进入组织架构全景**，便于快速理解各部职责并导航 |
| **默认端口** | `7891` | **`8080`** (更符合现代 Web 应用标准的默认端口) |
| **功能面板数** | 10 个 | **12 个** (新增组织架构与朝堂议政) |
| **部署架构优化** | 强依赖本地 OpenClaw 运行时配置 | **内置环境变量支持** (`CI=true DEBIAN_FRONTEND=noninteractive` 等)，增强 CI/CD 与容器化兼容性 |
| **UI 细节优化** | 纯净 CSS，基础卡片 | **现代化 CSS 渐变节点、精细悬浮动画 (Hover Scale)**，强化层级视觉辨识度 |
...
- **六部与院监**（户·礼·兵·刑·工·吏 + 翰林院 + 钦天监）负责专项执行与辅助调研
...
```bash
docker run -p 8080:8080 cft0808/sansheng-demo
```
打开 http://localhost:8080 即可体验军机处看板。
...
```bash
docker run --platform linux/amd64 -p 8080:8080 cft0808/sansheng-demo
```
...
flowchart TD
    ...
    Emperor["👑 皇上 (你)<br/>(Feishu / Telegram / Signal)"]:::emperor
    Taizi["🤴 太子 (taizi)<br/>消息分拣 / 旨意建任务"]:::taizi
    Zhongshu["📜 中书省 (zhongshu)<br/>接旨 → 规划 → 拆解"]:::sheng
    Menxia["🔍 门下省 (menxia)<br/>审议方案 → 准奏 / 封驳"]:::sheng
    Shangshu["📮 尚书省 (shangshu)<br/>派发任务 → 协调六部 → 汇总"]:::sheng
    Hanlin["📚 翰林院 (hanlin)<br/>调研 / 搜索"]:::hanlin
    Zaochao["📰 钦天监 (zaochao)<br/>天下要闻"]:::zaochao
    
    Hubu["💰 户部<br/>数据"]:::bu
    Libu["📝 礼部<br/>文档"]:::bu
    Bingbu["⚔️ 兵部<br/>工程"]:::bu
    Xingbu["⚖️ 刑部<br/>合规"]:::bu
    Gongbu["🔧 工部<br/>代码"]:::bu
    LHr["👔 吏部<br/>配置"]:::bu
...
| 📚 **翰林院** | `hanlin` | 调研、搜索、信息收集 | 网络搜索、深度调研、技术选型 |
| 📰 **钦天监** | `zaochao` | 每日早朝、新闻聚合 | 定时播报、天下要闻数据汇总 |

### 权限矩阵

> 不是想发就能发 —— 真正的分权制衡

| From ↓ \ To → | 太子 | 中书 | 门下 | 尚书 | 六部 | 吏部 | 翰林 | 钦天监 |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **太子** | — | ✅ | | | | | ✅ | ✅ |
| **中书省** | ✅ | — | ✅ | ✅ | | | | |
| **门下省** | | ✅ | — | ✅ | | | | |
...
```bash
# 添加远程 skill
curl -X POST http://localhost:8080/api/add-remote-skill \
...
# 查看所有远程 skills
curl http://localhost:8080/api/remote-skills-list
```
...
1. **检查 Agent 注册状态**：
```bash
curl -s http://127.0.0.1:8080/api/agents-status | python3 -m json.tool
```
...
```bash
# 手动触发巡检扫描（自动重试卡住的任务）
curl -X POST http://127.0.0.1:8080/api/scheduler-scan \
