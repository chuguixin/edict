...
def main():
    ...
    default_model = normalize_model(
        agents_cfg.get("defaults", {}).get("model", {}), "unknown"
    )
    ...
    for ag in agents_list:
        ...
        meta = ID_LABEL[ag_id]
        workspace = ag.get(
            "workspace", str(pathlib.Path.home() / f".openclaw/workspace-{ag_id}")
        )
        result.append(
            {
                "id": ag_id,
    ...
    EXTRA_AGENTS = {
        "taizi": {
            "model": default_model,
            "workspace": str(pathlib.Path.home() / ".openclaw/workspace-taizi"),
            "allowAgents": ["zhongshu"],
        },
        "main": {
            "model": default_model,
            "workspace": str(pathlib.Path.home() / ".openclaw/workspace-main"),
            "allowAgents": [
                "zhongshu",
                "menxia",
                "shangshu",
        ...
        "zaochao": {
            "model": default_model,
            "workspace": str(pathlib.Path.home() / ".openclaw/workspace-zaochao"),
            "allowAgents": [],
        },
        "libu_hr": {
            "model": default_model,
            "workspace": str(pathlib.Path.home() / ".openclaw/workspace-libu_hr"),
...
def sync_scripts_to_workspaces():
    ...
    for proj_name, runtime_id in _SOUL_DEPLOY_MAP.items():
        ws_scripts = (
            pathlib.Path.home() / f".openclaw/workspace-{runtime_id}" / "scripts"
        )
